...
LATEST: captured per-face sky/block lighting replaces full-sky local lighting and top-only column lighting. F9 K compares old/new on the same frozen scene; F10 defaults to captured lighting. Controlled under-platform error improves0.0656→0.0001, downward mountains only marginally; falling snow is an additional omitted feature. Both reference pairs pixel-identical. See docs/face-lighting.md for exact data, resource cost(+35MiB maximum for two snapshots) and limitations. Build/45 tests and runtime pass. Live far-pose GPU p95=9.658944ms, frame p95=12.2304ms with prior exclusions. Current logs face-light-final-build.log/face-light-verified-runtime.log. Client F10 on at far pose below, creative flight, small window. Next: smooth corner shading/AO, terrain geometry/coverage and hard128 viewing boundary. Existing projection/fog findings remain in docs/native-fog.md. Clouds in front of terrain, precipitation, daylight/settings/dimensions remain incomplete. Earlier repair metrics below are historical.
