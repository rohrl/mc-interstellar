...
## Current uncommitted experiment: early facing hints

TriangleFacing tags vertex B's unused flag slot for exact axis-plane right triangles; vertex A type and all positions/material attributes unchanged. Candidate skips provably back/parallel faces before vertex C fetch; original shaders ignore hints. X/Shift+X toggles/compares separate programs on cap16. Fixtures receive hints. facing-hints-build.log passes55 tests; new facing-hints-runtime.log launched after closing prior live client normally, setup/checks pending. See docs/facing-hints.md. Current accepted rollback7792557 is pushed. Need GPU26240 checks and matched1440p image/timing gates before retaining. Goal remains active toward at least30FPS in the slower terrain view.
