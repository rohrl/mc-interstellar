...
LATEST: repaired native sky capture state after pushed projection fix 4bd288b. Explicitly selects Minecraft's sky shader and sets/restores global projection/model-view/shader/colour; uses current tick interpolation. Nighttime fixed pairs show aligned sky gradient/moon/cloud pattern; repeated vanilla references are pixel-identical. Build/45 tests pass. Live far-pose GPU p95=9.701568 ms, frame p95=11.5768 ms, same timing exclusions. Details and capture IDs: sky render-state subsection in docs/native-appearance.md. Current logs sky-state-build.log, sky-state-runtime.log and sky-state-timing-runtime.log. Client F10 on at far pose below, creative flight, small window. Remaining: terrain shading/filtering, height-field geometry/coverage, and clouds in front of terrain (currently omitted); daylight/settings/dimensions still need coverage. The following initial-repair metrics are historical.
