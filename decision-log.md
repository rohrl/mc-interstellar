...
Same-capture repeated GPU medians improve4.00% down and3.46% wall, with improved p95;58.2% fewer moving node visits. Both heavy image pairs exact;52,480 optical cases/84 material cases/59 tests pass. Additional demo comparison exposes small horse-face overlap differences (.0125% pixels >8/255), consistent with the existing unresolved coplanar visitation-order limitation. Document this explicitly; no universal image-equivalence claim. Live~39FPS is a current-scene observation, not a causal comparison against the older, larger mob population.
...
Heavy same-capture GPU medians average24.514ms float/29.423ms packed:20.03% more time,16.69% less throughput. Wall regresses17.1%; tails also worsen. Node visits rise under0.5%, so decode, dependent loads/cache behaviour or register cost remain unisolated explanations. Reject this representation; do not generalize to every possible packing scheme or infer hardware stalls from counters. Preserve e62a71c on codex/quantized-node-bounds.
...
docs/gameplay-gravity-plan.md proposes exact equal-weight centres, shared bounded event-driven discovery/updates and `r_s/block=sqrt(3)/32` for the reference cube threshold. Preserve the existing exhibit under an explicit legacy calibration; its 64-block radius otherwise shrinks from8 to3.46. Subcritical rendering needs a finite interior approximation, not simply removing the current black-hole gate. Exterior optics and native world integration remain protected.
...
Subcritical optics use an explicit finite rational-lapse spherical interior matched to the Schwarzschild exterior; dedicated programs keep that solver out of the black-hole path. Thirty sampled rays compare the inverse-radius equation against an independent affine Hamiltonian reference. This is not a stellar EOS, nonspherical collapse model or arbitrary-ray certification.

Entity motion uses scaled Newtonian acceleration, a finite uniform interior, smooth local cutoff and bounded acceleration. Preserve native AI/contacts; substep ordinary/spectral arrows and thrown projectiles through native collision handling, with unscaled impact velocity and horizon ordering. Capture discards entities without drops or terrain damage. Players/mounted groups, tridents/homing projectiles and oversized source dynamics are excluded. A captured-mesh red/dim cue costs no extra pass and is explicitly stylized, not physical spectral transport.
...
Use a separate interstellar:arrows exhibit because the owner moved the gameplay exhibit's source; fixed launch sites there would no longer demonstrate the intended trajectories. Preserve existing builds. Four colour-marked dispensers fire calibrated native arrows without inventory/random spread. Native downward gravity, drag and collisions remain; the observed491.9-degree loop is temporary. Limit to12 tracked arrows, expire at8seconds, stop spawning without nearby viewers and remove stale demo projectiles on reload. Session on/off/once controls and a closer viewpoint make the experiment repeatable.

Start the inexpensive mob red/dim cue at3 horizon radii and increase its contrast. Do not equate this with physical spectral transport or an apparent frozen horizon image: ordinary-time mob motion remains, and delayed light/emission history stays deferred.
...
The later vanilla glowing-outline framebuffer contains straight-ray positions, so suppress it only when a curved world image was actually drawn this frame. Loading, out-of-range fallback and F10-off retain normal outlines. This avoids adding a misplaced outline overlay while proper curved outlines remain unimplemented. Do not claim restored foreground hands implement returning-light player-body images.
...
Use a separate glowing-only model tree and curved mask followed by a screen-space edge filter. Preserve native team colours and intentional through-wall visibility. Skip the pass entirely without glowing geometry. Do not overlay the vanilla straight-world silhouette. Use the same camera, metric and ray-quality controls as the underlying scene; no new lens approximation.

The actual camera-player skin/model is eligible after a ray has bent past pi around the source. This makes returning images possible while preventing the near head from occluding the first-person camera. Default resolution can reduce the image to thin arcs; current-pose geometry does not implement delayed emission history. Keep explicit body/weather session toggles.
