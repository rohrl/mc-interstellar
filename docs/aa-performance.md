...
## Rejected experiments and renewed30FPS minimum — 2026-09-19

The owner requested stopping when small gains require disproportionate complexity. Two further experiments do not justify production changes:

- [Surface-area terrain trees](experiments/terrain-sah.md): GPU p95 improves only1.9% wall/3.3% downward, while CPU tree construction costs3.7–4.1x as much. Rejected; preserved on `codex/terrain-sah-experiment` at `ba0bd73`.
- [Longer empty-space steps](empty-spans.md): a conversion-only variant saves0.5%. A stronger variant appears6.0% faster wall/2.8% downward against a uniform-disabled version of itself, but is **2.5% slower** against the separately compiled original, confirmed in reverse order. Rejected; preserved on `codex/empty-step-experiment` at `220b883`.

Production retains the b41b2f2 renderer and sharp2xAA. In the latest original-program wall measurements at1440p, median frame interval is49.5–49.6ms (~20FPS); another frozen downward scene measures82.6ms (~12FPS). Scene populations differ, so these are representative observations, not a cross-session improvement. **60FPS is not reached.** No AA, optical accuracy, coverage or animation reduction is accepted here.

The owner subsequently rejected stopping at20FPS and set an explicit30FPS minimum. Optimization continues; these rejected experiments do not exhaust viable approaches. Deferring temporary-hit shading still requires alpha tests and extra retained hit state; gain is unmeasured and compiler effects can erase it. Packed integer nodes require changes across builders/uploads/addressing. Compute/wavefront or hardware traversal would be larger renderer projects; temporal reconstruction requires explicit quality/motion acceptance. These remain proposals needing profiling and isolated experiments, not promised speedups. Next test deferring opaque shading until the final nearest hit, then reduce geometry fetch/traversal work; use separately compiled references and preserve alpha/cloud ordering.

The [precipitation assessment](precipitation-assessment.md) recommends deferral because transparent composition needs additional design/measurement. [Feature ideas](relativity-feature-ideas.md) favour clocks, bounded orbit probes and light-echo instruments without another expensive full-screen pass. Existing packaging, horizon-crossing and actual-player-body work remains on the roadmap.
