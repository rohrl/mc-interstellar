...
After inspecting a black-hole proxy, press **F10** and allow initial native terrain capture (about40 seconds at render distance12). Then explore with normal movement, animated mobs/clouds and the vanilla HUD. **F12** measures the pass; F10 returns to normal rendering. **Terrain remains captured at activation: block edits and chunk streaming are next.** Re-enabling F10 currently recaptures it. Interactions still use straight aim. [Current implementation and checks](docs/live-native-mesh.md) · [Earlier voxel prototype](docs/live-terrain.md).
