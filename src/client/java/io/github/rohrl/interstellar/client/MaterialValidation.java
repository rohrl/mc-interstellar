...
final class MaterialValidation {
    ...
    static int run(ShaderProgram shader,Runnable draw,MeshArena triangles,MeshArena nodes,MeshArena compact,boolean quads) {
        ...
        try {
            ...
            for(int kind=0;kind<7;kind++)for(int reverse=0;reverse<2;reverse++)for(int cache=0;cache<2;cache++) {
                ...
                if(!Float.isFinite(error) || error>2e-5f) {failures++;Interstellar.LOGGER.error("Material fixture mismatch: case={}, reverse={}, cache={}, actual=({},{},{}), error={}",kind,reverse,cache,pixels.get(0),pixels.get(1),pixels.get(2),error);}
            }
        } finally {
