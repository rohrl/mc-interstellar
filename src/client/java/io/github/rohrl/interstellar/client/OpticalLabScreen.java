...
public final class OpticalLabScreen extends Screen {
    ...
    @Override public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        previousFrame = now;
        if (playing) {
            radius = (float) Math.pow(Math.max(Math.pow(.35, 1.5), Math.pow(radius, 1.5)-1.5*seconds), 2.0/3);
    ...
    @Override public boolean keyPressed(int key, int scanCode, int modifiers) {
        if (key == GLFW.GLFW_KEY_B || key == GLFW.GLFW_KEY_V || key == GLFW.GLFW_KEY_F ||
                key == GLFW.GLFW_KEY_UP || key == GLFW.GLFW_KEY_DOWN || key == GLFW.GLFW_KEY_R || key == GLFW.GLFW_KEY_H) playing = false;
        ...
        if (key == GLFW.GLFW_KEY_B) {
            if (benchmark != null) { benchmark.close(); benchmark = null; }
                    ...
                    "%dx%d, r/rs=%.4f, lensing=%s, grid=%s, aligned=%s, falling=%s, lookBack=%s",
                    client.getWindow().getFramebufferWidth(), client.getWindow().getFramebufferHeight(), radius, lensing, grid, aligned, falling, lookBack));
            return true;
        }
        if (benchmark != null) { benchmark.close(); benchmark = null; }
        switch (key) {
            ...
            case GLFW.GLFW_KEY_R -> { radius = 8; lensing = true; grid = true; aligned = true; falling = false; playing = false; lookBack = false; }
